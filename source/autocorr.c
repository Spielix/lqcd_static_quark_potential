...
int main(int argc, char **argv) {
	...
	double ***data = malloc((par.L_t - 1) * sizeof(double **));

	for(int i = 0; i < par.L_t - 1; i++) 
        data[i] = malloc((par.L - 1) * sizeof(double *));
	for(int i = 0; i < par.L_t - 1; i++){
	...
	double **mu = malloc((par.L_t - 1) * sizeof(double *));
	double **std_dev = malloc((par.L_t - 1) * sizeof(double *));
	double **t_corr = malloc((par.L_t - 1) * sizeof(double *));
	double **err = malloc((par.L_t - 1) * sizeof(double *));
	for(int i = 0; i < par.L_t; i++){
		mu[i] = malloc((par.L - 1) * sizeof(double));
		std_dev[i] = malloc((par.L - 1) * sizeof(double));
		t_corr[i] = malloc((par.L - 1) * sizeof(double));
		err[i] = malloc((par.L - 1) * sizeof(double));
	}

	for (int i = 0; i < par.L_t - 1; i++) {
		for (int j = 0; j < par.L - 1; j++) {
			mu[i][j] = gsl_stats_mean(data[i][j], 1, par.n_configs);
		}
	}
	for (int i = 0; i < par.L_t - 1; i++) {
		for (int j = 0; j < par.L - 1; j++) {
